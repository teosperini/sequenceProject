...
int main(int argc, char *argv[]) {
	...
	if (provided < MPI_THREAD_MULTIPLE) {
		fprintf(stderr, "Errore: il livello di supporto per i thread MPI_THREAD_MULTIPLE non e' disponibile.\n");
		MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
	}
 ...
 */
	...
	if (rank == 0) {
		...
		#pragma omp parallel for reduction(+:total_requests)
	...
	for (int local_pat = 0; local_pat < local_pat_number; local_pat++) {
		...
		for (start = 0; start <= seq_length - local_pat_length[local_pat]; start++) {
			// Se il primo carattere non combacia, salto il confronto
			if (sequence[start] != local_pattern[local_pat][0]) continue;
	...
	// 1. Riduzione del numero totale di match trovati da tutti i processi
	MPI_Reduce(&local_matches, &global_matches, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
	// 2. Riduzione del conteggio delle occorrenze
	MPI_Reduce(seq_matches, global_seq_matches, seq_length, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

	// Allocazione degli array per la raccolta dei dati nel processo root (rank 0)
	int *recvcounts = malloc(size * sizeof(int)); // Numero di elementi ricevuti da ogni processo
	int *displs = malloc(size * sizeof(int)); // Offset per l'inserimento dei dati ricevuti
	// Ogni processo invia il numero di pattern trovati localmente al processo root (rank 0)
	MPI_Gather(&local_pat_number, 1, MPI_INT, recvcounts, 1, MPI_INT, 0, MPI_COMM_WORLD);

	if (rank == 0) {
		// Il processo root calcola gli offset per la raccolta dei dati
		displs[0] = 0;
		for (int i = 1; i < size; i++) {
			// L'offset di ogni processo è la somma degli elementi ricevuti dai processi precedenti
			displs[i] = displs[i - 1] + recvcounts[i - 1];
		}
	}
	// Ogni processo invia le posizioni trovate dei pattern e il rank 0 le raccoglie in global_pat_found
	MPI_Gatherv(local_pat_found, local_pat_number, MPI_UNSIGNED_LONG, global_pat_found, recvcounts, displs, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
	...
	#pragma omp parallel for reduction(+:checksum_matches) private(lind)
	for( lind=0; lind < seq_length; lind++) {
		if ( global_seq_matches[lind] != (int)NOT_FOUND ){
	...
	printf("Found start:");
	for( debug_pat=0; debug_pat<pat_number; debug_pat++ ) {
		printf( " %lu", global_pat_found[debug_pat] );
	}
	printf("\n");
	printf("-----------------\n");
	printf("Matches:");
